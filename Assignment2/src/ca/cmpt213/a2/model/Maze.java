...
/**
 * The class has the information to generate a maze.
 * The maze uses recursive backtracking implementing DFS
 * algorithm using a stack. This class also supports removal
 * of walls for additional paths through the maze without creating
 * 2x2 square of empty cells or walls.
 */
public class Maze {
    ...
    /**
     * Remove corner walls with an empty space
     */
    ...
    private void checkSideWall(Cell[][] maze) {
        for (int i = 2; i < MAZE_ROW - 3; i ++) {
            if ((maze[i - 1][MAZE_COL-2].getValue() != Value.EMPTY_SPACE || maze[i][MAZE_COL - 3].getValue() != Value.EMPTY_SPACE
                    || maze[i - 1][MAZE_COL - 3].getValue() != Value.EMPTY_SPACE) && (maze[i][MAZE_COL - 3].getValue() != Value.EMPTY_SPACE
                    || maze[i + 1][MAZE_COL - 2].getValue() != Value.EMPTY_SPACE || maze[i + 1][MAZE_COL - 3].getValue() != Value.EMPTY_SPACE)) {
    ...
    /**
     * Set each 2-d maze index to the cell objects value
     */
     ...
     * Randomizes ordering of numbers as directions
     */
